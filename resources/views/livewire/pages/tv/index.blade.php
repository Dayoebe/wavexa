<div>
        ...
        <div class="mx-auto max-w-7xl px-4 pb-8 pt-7 sm:px-6 sm:py-14 lg:px-8">
            ...
            <div x-data="{ open: false }">
                ...
                <form wire:submit="applyFilters" :class="open ? 'grid' : 'hidden sm:grid'" class="mt-3 gap-3 rounded-[26px] border border-cyan-200 bg-white p-3 shadow-xl shadow-cyan-950/5 sm:mt-7 sm:grid-cols-[2fr_1fr_1fr_auto]">
                    <input wire:model="q" placeholder="Search a TV channel" class="min-h-14 rounded-[18px] border-0 bg-stone-100 px-4 text-base outline-none">
                    <select wire:model="country" class="min-h-14 rounded-[18px] border-0 bg-stone-100 px-4 font-bold"><option value="">Every country</option>@foreach($countries as $item)<option value="{{ $item->iso_alpha_2 }}">{{ $item->name }}</option>@endforeach</select>
                    <select wire:model="sort" class="min-h-14 rounded-[18px] border-0 bg-stone-100 px-4 font-bold"><option value="name_asc">Name A–Z</option><option value="name_desc">Name Z–A</option><option value="country">Country A–Z</option></select>
    ...
    <section id="channels" class="py-10 sm:py-14">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8" wire:loading.class="opacity-60">
            <div class="flex items-end justify-between"><div><p class="text-[11px] font-extrabold uppercase tracking-[.2em] text-cyan-700">On air</p><h2 class="mt-1 text-2xl font-extrabold sm:text-3xl">Choose a channel</h2></div>@if($q !== '' || $country !== '' || $sort !== 'name_asc')<button wire:click="clearFilters" class="rounded-full bg-stone-200 px-3 py-2 text-xs font-bold">Clear filters</button>@endif</div>
            @if($channels->isEmpty())
