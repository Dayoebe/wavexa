        ...
        <header class="sticky top-0 z-40 border-b border-stone-200 bg-stone-50/95 backdrop-blur-xl">
            <div class="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
                ...
                <nav class="hidden items-center gap-1 rounded-full border border-stone-200 bg-white p-1 text-sm font-semibold shadow-sm md:flex" aria-label="Primary navigation">
                    <a wire:navigate href="{{ route('home') }}" class="rounded-full px-5 py-2 text-slate-600 hover:bg-stone-100">Discover</a>
                    <a wire:navigate href="{{ route('radio.index') }}" class="rounded-full px-5 py-2 {{ request()->routeIs('radio.*') ? 'bg-orange-500 text-white' : 'text-slate-600 hover:bg-orange-50' }}">Radio</a>
                    <a wire:navigate href="{{ route('tv.index') }}" class="rounded-full px-5 py-2 {{ request()->routeIs('tv.*') ? 'bg-cyan-600 text-white' : 'text-slate-600 hover:bg-cyan-50' }}">TV</a><span class="rounded-full px-5 py-2 text-slate-400">Podcasts</span>
                </nav>
