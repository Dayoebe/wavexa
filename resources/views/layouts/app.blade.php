    ...
    <body class="min-h-screen bg-stone-50 font-sans text-slate-950 antialiased">
        ...
        <div x-cloak x-show="searchOpen" x-transition.opacity class="fixed inset-0 z-[80] bg-slate-950/45 p-4 backdrop-blur-sm" @click.self="searchOpen = false">
            <div x-show="searchOpen" x-transition class="mx-auto mt-16 max-w-2xl rounded-[28px] bg-white p-4 shadow-2xl sm:mt-28 sm:p-6">
                ...
                <form action="{{ route('search') }}" class="mt-5 flex rounded-2xl border border-stone-300 bg-stone-50 p-2"><input x-ref="globalSearch" name="q" class="min-w-0 flex-1 bg-transparent px-3 outline-none" placeholder="Station, channel, podcast, country, genre, or language"><button class="rounded-xl bg-slate-950 px-5 py-3 text-sm font-bold text-white">Search Wavexa</button></form>
