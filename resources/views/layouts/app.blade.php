    ...
    <body class="min-h-screen bg-stone-50 font-sans text-slate-950 antialiased">
        ...
        <div x-cloak x-show="searchOpen" x-transition.opacity class="fixed inset-0 z-[80] bg-slate-950/45 p-4 backdrop-blur-sm" @click.self="searchOpen = false">
            <div x-show="searchOpen" x-transition class="mx-auto mt-16 max-w-2xl rounded-[28px] bg-white p-4 shadow-2xl sm:mt-28 sm:p-6">
                ...
                <div class="mt-4 grid grid-cols-2 gap-3"><a wire:navigate @click="searchOpen = false" href="{{ route('radio.index') }}" class="rounded-2xl bg-orange-50 p-4 text-sm font-bold text-orange-800">Browse all radio →</a><a wire:navigate @click="searchOpen = false" href="{{ route('tv.index') }}" class="rounded-2xl bg-cyan-50 p-4 text-sm font-bold text-cyan-800">Browse all TV →</a></div>
            </div>
        </div>
        </div>
        ...
        <nav class="fixed inset-x-3 bottom-3 z-50 grid grid-cols-4 rounded-[24px] border border-stone-200 bg-white/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 shadow-2xl shadow-slate-900/20 backdrop-blur-xl md:hidden" aria-label="Mobile navigation" x-data>
            ...
            <button type="button" @click="document.querySelector('header button[aria-label=\'Open search\']')?.click()" class="flex min-h-12 flex-col items-center justify-center gap-1 rounded-2xl text-[10px] font-bold text-slate-500"><svg viewBox="0 0 24 24" class="size-5" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/></svg>Search</button>
        </nav>
