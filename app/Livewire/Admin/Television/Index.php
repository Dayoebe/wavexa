...
class Index extends Component
{
    ...
    #[Url(except: '')] public string $country = '';
    #[Url(except: 'active')] public string $records = 'active';

    public function updated(string $property): void
    {
        if (in_array($property, ['q', 'status', 'country', 'records'], true)) $this->resetPage();
    }

    public function clearFilters(): void
    {
        $this->reset(['q', 'status', 'country']); $this->records = 'active'; $this->resetPage();
    }

    public function delete(int $id): void
    {
        $this->channel($id)->delete(); session()->flash('success', 'Channel moved to deleted records.');
    }

    public function restore(int $id): void
    {
        $this->channel($id, true)->restore(); session()->flash('success', 'Channel restored.');
    }

    public function forceDelete(int $id): void
    {
        $channel = $this->channel($id, true); abort_unless($channel->trashed(), 422); $channel->forceDelete(); session()->flash('success', 'Channel permanently deleted.');
    }

    public function render(): View
    {
        $query = Media::query()->where('type', MediaType::Television)->with(['country', 'tvChannel', 'primaryStream', 'artworks']);
        if ($this->records === 'deleted') $query->onlyTrashed();
        ...
        $countries = Country::query()->whereHas('media', fn (Builder $query) => $query->where('type', MediaType::Television))->orderBy('name')->get(['id', 'name']);
